...
def test_create_items_openpack(va_login_fixture):
...
@pytest.mark.parametrize("va_login_fixture", ["va"], indirect=True)  # account 파라미터 설정
def test_create_items_prepack(va_login_fixture):
    page = va_login_fixture    # 로그인된 페이지 사용
    page.wait_for_timeout(3000) #3초 대기
...
# def test_create_items_prepack1(login_fixture):
#     page = login_fixture    # 로그인된 페이지 사용
#     va_create_items_prepack1(page)  # prepack 추가 생성
